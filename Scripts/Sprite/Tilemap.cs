...
public class Tilemap : MonoBehaviour
{
    ...
    public void BuildMap()
    {
        ...
        bool[] n_coll = new bool[width * height];

        for(int i = 0; i < width; ++i)
        {
            for (int j = 0; j < height; ++j)
            {
                if ( i < _internalWidth && j < _internalHeight)
                {
                    int oldIdx = i * _internalHeight + j;
                    if (indexes != null && indexes.Length > oldIdx)
                    {
                        n_idx[i * height + j] = indexes[oldIdx];
                    }

                    if (collisions != null && collisions.Length > oldIdx)
                    {
                        n_coll[i * height + j] = collisions[oldIdx];
                    }
                }
                else
                {
                    n_idx[i * height +j] = -1;
                    n_coll[i * height + j] = false;
                }
            }
        }
    ...
    {
        return !collisions[x * _internalHeight + y];
    }

    public void SetCollision(int x, int y, bool collide)
    {
        collisions[x * _internalHeight + y] = collide;
    }
    ...
    {
        if (indexes == null || width != _internalWidth || height != _internalHeight)
            BuildMap();
